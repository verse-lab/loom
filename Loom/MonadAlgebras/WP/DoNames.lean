...
structure MProdWithNames (α β : Type u) where
  /-- The first element of a pair. -/
  fst : α
  /-- The second element of a pair. -/
  snd : β

  fstName : Lean.Name := default
  sndName : Lean.Name := default
...
abbrev Var := Name × Syntax  -- TODO: should be `Ident`
...
private def varSetToArray (s : VarSet) : Array Var :=
  s.fold (fun xs n x => xs.push ⟨n, x⟩) #[]

private def varsToMessageData (vars : Array Var) : MessageData :=
  MessageData.joinSep (vars.toList.map fun n => MessageData.ofName (n.2.getId.simpMacroScopes)) " "

partial def CodeBlocl.toMessageData (codeBlock : CodeBlock) : MessageData :=
  let us := MessageData.ofList <| (varSetToArray codeBlock.uvars).toList.map (MessageData.ofSyntax ·.2)
...
def mkAuxDeclFor {m} [Monad m] [MonadQuotation m] (e : Syntax) (mkCont : Syntax → m Code) : m Code :=
  withRef e <| withFreshMacroScope do
    let y ← `(y)
    ...
    let k ← mkCont y
    return .decl #[(default, y)] doElem k
...
partial def convertTerminalActionIntoJmp (code : Code) (jp : Name) (xs : Array Var) : MacroM Code :=
  let rec loop : Code → MacroM Code
    ...
    | .action e              => mkAuxDeclFor e fun y =>
      ...
      let jmpArgs := xs.push ⟨default, y⟩
      return Code.jmp ref jp (jmpArgs.map (·.snd))
...
def insertVars (rs : VarSet) (xs : Array Var) : VarSet :=
  xs.foldl (fun rs x => rs.insert x.2.getId x.2) rs

def eraseVars (rs : VarSet) (xs : Array Var) : VarSet :=
  xs.foldl (·.erase ·.2.getId) rs

def eraseOptVar (rs : VarSet) (x? : Option Var) : VarSet :=
  match x? with
  | none   => rs
  | some x => rs.insert x.2.getId x.2
...
/--
We use `MProdNames` instead of `Prod` to group values when expanding the
`do` notation. `MProdNames` is a universe monomorphic product.
...
def returnToTerm (val : Syntax) : M Syntax := do
  ...
  let u ← mkUVarTuple
  match ctx.kind with
  | .regular         => if ctx.uvars.isEmpty then ``(Pure.pure $val) else ``(Pure.pure (MProdNames.mk $val $u))
  | .forIn           => ``(Pure.pure (ForInStep.done $u))
  | .forInWithReturn => ``(Pure.pure (ForInStep.done (MProdNames.mk (some $val) $u)))
...
def continueToTerm : M Syntax := do
  ...
  | .forInWithReturn => ``(Pure.pure (ForInStep.yield (MProdNames.mk none $u)))
...
def breakToTerm : M Syntax := do
  ...
  | .regular         => unreachable!
  | .forIn           => ``(Pure.pure (ForInStep.done $u))
  | .forInWithReturn => ``(Pure.pure (ForInStep.done (MProdNames.mk none $u)))
...
def actionTerminalToTerm (action : Syntax) : M Syntax := withRef action <| withFreshMacroScope do
  ...
  let u ← mkUVarTuple
  match ctx.kind with
  | .regular         => if ctx.uvars.isEmpty then pure action else ``(Bind.bind $action fun y => Pure.pure (MProdNames.mk y $u))
  | .forIn           => ``(Bind.bind $action fun (_ : PUnit) => Pure.pure (ForInStep.yield $u))
  | .forInWithReturn => ``(Bind.bind $action fun (_ : PUnit) => Pure.pure (ForInStep.yield (MProdNames.mk none $u)))
  | .nestedBC        => unreachable!
    ...
    if h : doForDecls.size > 1 then
      ...
      if hasReturn forInBodyCodeBlock.code then
        ...
        let forInTerm ← if let some h := h? then
          annotate doFor
            (← `(for_in'% $(xs) (MProdNames.mk (none : $optType) $uvarsTuple) fun $x $h (r : MProdNames $optType _) => let r := r.2; $forInBody))
        else
          annotate doFor
            (← `(for_in% $(xs) (MProdNames.mk (none : $optType) $uvarsTuple) fun $x (r : MProdNames $optType _) => let r := r.2; $forInBody))
